...
// persona - helper privati
bool persona::isValidEmail(const std::string& email)
{
    if (email.empty() || email.length() > 254 || email.length() < 5) return false;
...
bool persona::isValidDateString(const std::string& date)
{
    ...
    try
    {
        ...
        if (meseVal < 1 || meseVal > 12) return false;
        int giorniMese[] = {31,28,31,30,31,30,31,31,30,31,30,31};
        if (anno % 4 == 0 && (anno % 100 != 0 || anno % 400 == 0)) giorniMese[1] = 29;
        if (giorno < 1 || giorno > giorniMese[meseVal - 1]) return false;
        return true;
    }
    catch (...) { return false; }
}
...
std::string persona::hashPassword(const std::string& password)
{
    ...
    for (unsigned char c : toHash) hash = ((hash << 5) + hash) + c;
    ...
    : utente(userId), nascita(nas)
{
    ...
    if (!isValidPassword(password)) throw InvalidPasswordException("La password deve contenere almeno 8 caratteri, una maiuscola, una minuscola, una cifra e un carattere speciale (!@#$%^&*)");
...
{
    if (!isValidEmail(newEmail)) throw InvalidEmailException(newEmail);
    email = newEmail;
}

void persona::setPassword(const std::string& newPassword)
{
    if (!isValidPassword(newPassword)) throw InvalidPasswordException("La password deve contenere almeno 8 caratteri, una maiuscola, una minuscola, una cifra e un carattere speciale (!@#$%^&*)");
    passwordHash = hashPassword(newPassword);
}

void persona::setNome(const std::string& newNome)
{
    if (newNome.empty()) throw std::invalid_argument("Il nome non può essere vuoto");
    nome = newNome;
}

void persona::setDataNascita(const dataNascita& newData)
{
    if (!newData.isValidBirthDate()) throw std::invalid_argument("La data di nascita non è valida");
    nascita = newData;
}

bool persona::verifyPassword(const std::string& password) const { return hashPassword(password) == passwordHash; }

void persona::updateProfile(const std::string& email, const std::string& nome, const dataNascita* dataNas)
{
    if (!email.empty() && !isValidEmail(email)) throw InvalidEmailException(email);
    if (!nome.empty() && nome.length() == 0) throw std::invalid_argument("Il nome non può essere vuoto");
    if (dataNas != nullptr && !dataNas->isValidBirthDate()) throw std::invalid_argument("La data di nascita non è valida");
    if (!email.empty()) this->email = email;
    if (!nome.empty()) this->nome = nome;
    if (dataNas != nullptr) this->nascita = *dataNas;
}

void persona::gestionePolicy()
{
    // placeholder: implementare UI/logic con Qt se necessario
}
